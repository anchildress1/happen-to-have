...
- **FR-002**: An answer review MUST perform two parallel calls on the original recording: content processing, and one judgment call carrying crisis, illegal-or-dangerous, and relevance.
- **FR-003**: A question review MUST perform the same two parallel calls. Relevance MUST NOT be evaluated for a question; the judgment call returns it as null rather than the system making a third call.
...
- **FR-008a**: Content processing MUST remain its own call and MUST NOT be merged into the judgment call. The split follows the provider's fault line: content processing reproduces the recording as text and was measured returning an empty candidate on recordings the judgment call handled cleanly. Merged, one block destroys every judgment and an unlawful recording renders as a processing failure instead of Withheld.
- **FR-008a1**: The three judgments MAY share one call. Measured across the fixture set, a merged judgment call matched separate calls on accuracy, blocked on nothing, and named the failing signal correctly in every case. The prior requirement that each judgment take its own call was asserted rather than measured, and the measurement does not support it.
- **FR-008b**: The **content-processing call** MUST set every safety category to a threshold that never blocks. Its output reproduces the recording as text, so it is the call the provider's filter actually trips, and a block there destroys a legitimate contribution while returning no reason and no ratings — retry cannot recover it. The **three boolean checks** MUST leave the provider's default thresholds in place: their output is a single bit, they rarely trip the filter, and a block there can only ever withhold a permit, never grant one. A provider block is therefore a conservative backstop against this system's own check being wrong.
- **FR-008b1**: A response with no candidate MUST be treated as a provider fault and retried under FR-038, never read as a verdict. The absence of a candidate carries no reason, and the same recording was observed blocking on one run and passing on another; a gate that flips on provider weather is not a gate. A blocked check therefore cannot publish, which is the property that makes leaving the default thresholds safe.
- **FR-008c**: The illegal-or-dangerous decision MUST be produced by the judgment call as its own field, never read from provider metadata. The spike settled this: the provider returns **no safety ratings at all** on this path — the field is absent from the response at a never-block threshold, at default thresholds, and with no safety configuration supplied. There is no free signal to read. Evidence: [spike-002-guardrails](../../docs/spike-002-guardrails.md).
- **FR-008d**: A crisis judgment MUST exist as its own field with its own instructions. No built-in category covers self-harm or crisis, and content moderation answers a different question — *is this content harmful to distribute* — than crisis, which asks *is this person in trouble*. The spike confirmed this at the provider's default guardrails: every crisis recording in the test set, including one naming a method and a written letter, was transcribed and returned clean.
- **FR-008e**: The judgment call MUST name which signal rejected the contribution rather than leaving it inferred from which boolean flipped. A reason reconstructed from booleans is a guess about the judge's reasoning; a reason the judge states is the judge's own. The transcript of a withheld contribution is never published, so this is the only thing left that selects the correct reason text.
- **FR-008h**: The judgment call MUST report the recording's audio quality — audible, unintelligible, or silent. Where content processing is lost to a provider block, this is the only signal left that can select among the content Withheld variants. It MUST NOT authorise publication: a lost transcript can never publish.
