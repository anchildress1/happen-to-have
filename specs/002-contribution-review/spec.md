...
- **FR-008a**: Every signal MUST have its own call. Measured on twenty understated-crisis recordings the prompt had never seen: on the content tier a merged call caught 9 of 10 and dedicated calls caught 10 of 10, the same single recording missed on all three merged runs; on the cheap tier the same split carries 2–3 of 10 to 8 of 10. A call holding several jobs stops doing the subtle one, and crisis is the subtle one — the tier decides how much that costs.
...
- **FR-008a1**: The crisis call MUST run on the content-processing tier rather than the cheap one. This is the larger of the two measured effects: dedicated, 8 of 10 on the small model against 10 of 10 on the large one; merged, 2–3 of 10 against 9 of 10. Crisis is the only signal whose failure causes harm outside the software.
...
- **Checks stay separate**: each signal is its own call, and this is measured rather than
  asserted — though the margin depends on the tier. On the cheap model merging costs five or six
  crisis detections in ten; on the shipped model it costs one, reproducibly, on the same
  recording every run. A call holding several jobs stops doing the subtle one.
