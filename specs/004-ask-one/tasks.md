...
- [ ] T018 ⚠️ **BLOCKED — route returns 500 with a session present.** [US2] Integration-test in `tests/integration/question-publish.test.ts` **both directions of the duration rule**: `61`, `0`, `-1` and a non-integer are each refused before any provider call is made — assert the faked client recorded zero invocations — SC-007, FR-006a; and a **one-second question publishes**, because FR-008 forbids a minimum and a bounds test alone reads like one
...
- [ ] T049 **`POST /api/ask` returns 500 when a session with an unspent ask submits.** Reproduced from the browser in `tests/e2e/ask.spec.ts`; the no-session path returns `401` correctly and every stubbed-UI path is green, so the failure is in the authenticated branch — after `findQuestionBySubmission` and `readAskEligibility`, before the duration refusal renders. Module imports are ruled out: the 401 path loads the same graph. **This blocks T018**, which is the server-side proof of SC-007, and it means the duration ceiling is currently enforced only by the route's own code and the column CHECK, neither of which is asserted end to end.
