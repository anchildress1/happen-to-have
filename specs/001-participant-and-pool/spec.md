...
**Input**: AI handoff "Happen to Have?" revision 4 — Arrival and Question selection sections, anonymous identity model, seeded pool requirement.
...
1. **Given** a question is displayed, **When** the participant chooses `Try another question`, **Then** a different open question is displayed.
...
4. **Given** the participant just skipped a question, **When** the next question is selected, **Then** the question they just skipped is not the one presented.
5. **Given** the participant has skipped every currently eligible question, **When** they skip again, **Then** previously skipped questions may be presented again rather than showing an empty pool.
...
**Independent Test**: Create a participant with one authored question and one answered
question, then request questions repeatedly and confirm neither is ever presented.
...
2. **Given** a participant who has already answered a question, **When** any question is selected for them, **Then** that question is never presented to them again, regardless of whether their answer was published or withheld.
...
- **Seeded questions**: the pool is populated before launch. Seed questions behave exactly like participant questions for selection and closure, but are attributed to identities that never open the site.
- **Every question already answered by this participant**: treated as an empty pool, not as a reason to re-present an answered question.
...
- **FR-016**: The system MUST NOT present a participant a question they have already answered, whether that answer was published or withheld.
- **FR-017**: The system MUST NOT present a question that has been closed for further routing.
- **FR-018**: The system MUST prefer questions with fewer published answers when selecting which question to present.
...
- **FR-024**: The system MUST NOT present the question a participant just skipped as the immediately next question.
- **FR-025**: A skipped question MUST remain permanently eligible for that participant and MAY be presented again later.
...
- **FR-028**: Seeded questions MUST be attributed to identities that never receive a personal history view.
...
- **SC-002**: Zero participants are ever presented their own question or a question they already answered, across the full selection test set including repeated requests.
- **SC-003**: A participant can skip at least twenty consecutive times without a repeated question, a penalty, or a change to ask eligibility.
- **SC-004**: Across one hundred selections against a seeded pool, questions with fewer published answers are presented more often than questions with more, demonstrating the preference is active.
...
- **Seed authorship**: seeded questions are authored by a person before launch and attributed to placeholder identities. They are real questions, not lorem text, because they are the first thing anyone reads.
- **Skip memory**: skip history is remembered for the duration of the session only. It prevents immediate repeats; it is not durable state worth persisting.
- **Selection determinism**: the fewer-answers preference is a bias, not a strict ordering. Some randomness is expected so two participants arriving together do not receive identical questions.
