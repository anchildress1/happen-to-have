...
- [x] T013 [P] Add `.github/workflows/ci.yml` running install, format-check, lint, typecheck, test, integration test, and build (SonarCloud analyses via `.sonarcloud.properties`, not a workflow step); job-level permissions, timeouts, concurrency group, and `paths-ignore`
